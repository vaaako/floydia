...
vec3<float> PerspectiveCamera::calc_move_dir(const vec3<float>& dir) noexcept {
	...
	vec3<float> world_dir = this->forward * (-dir.z) + this->right * -dir.x; // Invert Z so -Z = forward
	// Normalize only horizontal (XZ) to prevent faster diagonal movement
	if(glm::length(world_dir) > 0.0f) world_dir = glm::normalize(world_dir);
	return world_dir;
}

void PerspectiveCamera::free_fly(const vec3<float>& dir, const float velocity) noexcept {
	this->position += (this->calc_move_dir(dir) + this->up * dir.y) * velocity;
}

void PerspectiveCamera::rotate(const vec2<float>& delta) noexcept {
	this->yaw   +=  delta.x * this->sensitivity;
	this->pitch += -delta.y * this->sensitivity; // -Y so UP/DOWN is correct
	this->update_vectors();
}
...
void PerspectiveCamera::update_vectors() noexcept {
	...
	this->right = glm::normalize(glm::cross({ 0.0f, 1.0f, 0.0f }, this->forward));
